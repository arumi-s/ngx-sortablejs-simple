...
import { SortablejsBindings } from './sortable-bindings';
...
export class SortablejsDirective implements OnInit, OnChanges, OnDestroy {
	...
	constructor(
		@Optional() @Inject(SORTABLEJS_CONFIG) private globalConfig: Options,
	...
	ngOnChanges(changes: { [prop in keyof SortablejsDirective]: SimpleChange }) {
		...
		if (optionsChange && !optionsChange.isFirstChange()) {
			...
			(Object.keys(currentOptions) as (keyof Sortable.Options)[]).forEach((optionName) => {
	...
	private proxyEvent(eventName: KeysOfType<Required<Options>, (event: SortableEvent) => void>, ...params: any[]) {
		this.zone.run(() => {
			...
			if (this.optionsWithoutEvents && this.optionsWithoutEvents[eventName]) {
				(this.optionsWithoutEvents[eventName] as any).apply(null, params);
			}
		});
	}
	...
	private get overriddenOptions(): Options {
		...
		return {
			onAdd: (event: SortableEvent) => {
				...
				this.proxyEvent('onAddOriginal' as 'onAdd', event);
			},
